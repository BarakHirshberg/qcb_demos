...
The apps are listed in the course's order of appearance.

- **Particle in a box** (infinite square well) — one app with four modes:
  1D (`E_n=n²h²/8mL²`), 2D (`E=(nₓ²+n_y²)…`, degeneracy + drumhead surface),
  two non-interacting particles in 1D (same product math; a joint-vs-conditional
  demo showing the particles are uncorrelated), and two interacting particles in
  1D (screened Coulomb, solved numerically by configuration interaction —
  correlation appears as depletion along `x₁=x₂`).
